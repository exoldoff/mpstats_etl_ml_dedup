...
| Pipeline processing | `docs/PIPELINE_OVERVIEW.md`, `docs/AI_INDEX.md` | `pipeline/services/`, `pipeline/repositories/`, `pipeline/data_quality/` | Не менять UI и analytics/output columns без запроса | `python3 -m compileall pipeline classifiers mpstats_app` плюс релевантный pytest |
