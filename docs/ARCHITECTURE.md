...
- Если `brand` отсутствует у нескольких строк и title совпадает после
  лёгкой нормализации регистра/пробелов, такие строки схлопываются в один
  research-record до embedding blocking. Это детерминированный pre-processing,
  чтобы одинаковые безбрендовые SKU не занимали соседние места в FAISS и не
  путали downstream-разметку.
