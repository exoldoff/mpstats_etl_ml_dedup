...
- Production processing и research-ноутбуки фильтруют хвост продаж:
  строки с `Продажи <= 0` отбрасываются, затем внутри каждого среза
  остаются верхние 80% положительных строк (`Продажи` не ниже нижнего
  20%-квантиля). В ноутбуках это дополнительно применяется при чтении
  старых `mpstats_products` из DuckDB.
